...
def printPictures():
    ...
    print("INSERT INTO PICTURE VALUES")
    counter = 0
    letter = "a"
    for i in range(0, len(user_fname)):
        for j in range(0, len(user_lname)):
            user = [user_fname[i], user_lname[j]]
            print("(\"imgur.com/" + str(counter) + ".jpg\", \"Seller Image\", \"Profile Picture for " + user[0] + " "
                  + user[1] + "\", NULL),")
            counter += 1
    ...
    for downloadURL in downloadURL_arr:
        ...
        if len(downloadURL) % 2 == 0:
            print("(\"imgur.com/ipitem#" + str(counter) + "v2.jpg\", \"IP Item Picture\", \"Second Item Picture for IP Item found at "
                  + downloadURL + "\", \"" + downloadURL + "\"),")
            if downloadURL.find('15') != -1:
                print("(\"imgur.com/ipitem#" + str(counter) + "v3.jpg\", \"IP Item Picture\", \"Third Item Picture for IP Item found at "
                      + downloadURL + "\", \"" + downloadURL + "\"),")
                print("(\"imgur.com/ipitem#" + str(counter) + "v4.jpg\", \"IP Item Picture\", \"Fourth Item Picture for IP Item found at "
                      + downloadURL + "\", \"" + downloadURL + "\"),")
                print("(\"imgur.com/ipitem#" + str(counter) + "v5.jpg\", \"IP Item Picture\", \"Fifth Item Picture for IP Item found at "
                      + downloadURL + "\", \"" + downloadURL + "\"),")
        counter += 1
...
def generateBTCWallets():
    cur = 11111111111111111111111111111111
    for i in range(0, 20):
...
def printIP():
    letter = "a"
    for i in range(0, 20):
        print("(\"tinyurl.com/" + str(letter) + "\", DateTime(\"now\"), \"Title " + str(letter) + "\", \"Categ " + str(
            letter) + "\", \"descript " + str(letter) + "\", \"jpg\"),")
...
def printBuyers():
    letter = "a"
    btc_index = 0
    print("INSERT INTO BUYER VALUES")
    for i in range(0, len(user_fname)):
        for j in range(0, len(user_lname)):
            user = [user_fname[i], user_lname[j]]
...
def printStore():
    ...
    wallet_key = 21111111111111111111111111111111
    r_num = 100000000
    for i in range(0, len(user_fname)):
        for j in range(0, len(user_lname)):
